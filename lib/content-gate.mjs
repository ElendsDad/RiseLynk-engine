...
import { LEAD_ATTRIBUTION_KEYS } from "./lead-attribution.mjs";
...
// intake columns are emitted (name, email, opted-in phone/message, source,
// lead-source attribution keys, and the reserved honeypot / Turnstile keys), so
// nothing the gate sends is silently folded or dropped by the intake mapper.
// The honeypot `website` value and the Turnstile token pass through UNTOUCHED:
// the server-side trap and verifier must see exactly what the browser sent.
// Attribution values pass through raw here; lib/contact-intake.mjs sanitizes
// them before foldExtras.
/**
...
export function gateLeadBody(data, gate) {
  ...
  for (const key of LEAD_ATTRIBUTION_KEYS) {
    if (key in d && d[key]) body[key] = String(d[key]);
  }
